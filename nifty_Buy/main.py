        ...
        for timeData in df.index:
            ...
            if not self.openPnl.empty:
                for index, row in self.openPnl.iterrows():
                    try:
                        ...
                        if row['EntryPrice'] > row['CurrentPrice'] and len(self.openPnl) == 1:
            ...
            if not self.openPnl.empty and ((timeData-60) in df.index):
                for index, row in self.openPnl.iterrows():
                    ...
                    elif (df.at[lastIndexTimeData[1], "callExit"] == "callExit") and row['EntryPrice'] < row['CurrentPrice']:
                        exitType = "callExit"
                        self.exitOrder(index, exitType, row["CurrentPrice"])

                    elif symSide == "PE" and len(self.openPnl) == 1:
                        exitType = "putExit"
                        self.exitOrder(index, exitType, row["CurrentPrice"])
