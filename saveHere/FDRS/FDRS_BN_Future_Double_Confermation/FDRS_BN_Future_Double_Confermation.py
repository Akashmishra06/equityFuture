...
class FDRS_BN_Future_Double_Confermation(baseAlgoLogic):
    def runBacktest(self, portfolio, startDate, endDate):
        if self.strategyName != "FDRS_BN_Future_Double_Confermation":
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        df['longEntry'] = np.where((df['rsi'] > 70) & (df['prev_rsi'] > 70) & (df['prev_prev_rsi'] < 70), "longEntry", "")
        df['longExit'] = np.where((df['prev_rsi'] < 30) & (df['rsi'] < 30) & (df['prev_prev_rsi'] > 30), "longExit", "")
...
if __name__ == "__main__":
    ...
    devName = "NA"
    strategyName = "FDRS_BN_Future_Double_Confermation"
    version = "v1"
    ...
    algoLogicObj = FDRS_BN_Future_Double_Confermation(devName, strategyName, version)
