...
class FDRS_Single_Confermation_RSI_7(baseAlgoLogic):
    def runBacktest(self, portfolio, startDate, endDate):
        if self.strategyName != "fdrs_70_30_15Min":
    ...
    def backtest(self, stockName, startDate, endDate):
        ...
        df['prev_h'] = df['h'].shift(1)
        df['prev_l'] = df['l'].shift(1)
        ...
        for timeData in df.index:
            ...
            for index, row in stockAlgoLogic.openPnl.iterrows():
                if lastIndexTimeData in df.index:
                    ...
                    if stockAlgoLogic.humanTime.time() >= time(15,15):
                        ...
                        elif (row['EntryPrice']*0.997) > df.at[lastIndexTimeData, "c"]:
                            exitType = "BuyStopLoss"
                            stockAlgoLogic.exitOrder(index, exitType, (df.at[lastIndexTimeData, "c"]))
                        ...
                        elif (row['EntryPrice']*1.003) < df.at[lastIndexTimeData, "c"]:
                            exitType = "SELLStopLoss"
                            stockAlgoLogic.exitOrder(index, exitType, (df.at[lastIndexTimeData, "c"]))
            ...
            if (lastIndexTimeData in df.index) & (stockAlgoLogic.openPnl.empty) & (stockAlgoLogic.humanTime.time() > time(9, 30)) & (stockAlgoLogic.humanTime.time() < time(15, 15)):
                ...
                if (df.at[lastIndexTimeData, "longEntry"] == "longEntry") and (df.at[lastIndexTimeData, "c"] > df.at[lastIndexTimeData, "o"]): # & (df.at[lastIndexTimeData, "c"] > df.at[lastIndexTimeData, "prev_h"])
...
if __name__ == "__main__":
    ...
    algoLogicObj = FDRS_Single_Confermation_RSI_7(devName, strategyName, version)
